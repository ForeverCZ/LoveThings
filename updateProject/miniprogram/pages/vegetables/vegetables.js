...
Page({
  ...
  flowerCloud: function(imgbase) {
    ...
    wx.cloud.callFunction({
      ...
      console.log(res)
      console.log(res.result.vegetables.result)
      let botanyimg = res.result.vegetables.result;

      //map()返回一个新数组  取小数点
      let botanydata = botanyimg.map(item => {
        let name = item.name;
        let calorie = item.calorie;
        let score = parseFloat(item.probability).toFixed(3);
  ...
  shre: function() {
    ...
    let aicalorie = this.data.botanyimg[0].calorie;
    if (aicalorie == null) {
      aidata = aidata;
    } else {
      aidata = aidata + aicalorie + "卡路里/100g"
    }
